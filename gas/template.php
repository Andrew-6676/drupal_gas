<?php
function gas_menu_tree__main_abon($variables) {
  return '<ul class="abon">' . $variables['tree'] . '</ul>';
}

function gas_preprocess_page(&$variables, $hook) {
  // echo '<pre>';
  //   print_r($variables['node']->nid);
  // echo '</pre>';
  if (isset($variables['node'])) {
    if ($variables['node']->nid==8) {
      $js = drupal_get_path('theme', 'gas') . '/js/podpis.js';
      drupal_add_js($js);
    }
  }
  // if (isset($variables['node'])) {
...
function gas_views_pre_render(&$view) {
  //if ($view->name=='vakansii') {
   // echo '-----'.$view->name;
    $css = drupal_get_path('theme', 'gas') . '/css/views/'.$view->name.'.css';
    if (file_exists($css)) {
      drupal_add_css($css, array('group' => CSS_THEME, 'type' => 'file'));
    }
    $js = drupal_get_path('theme', 'gas') . '/js/views/'.$view->name.'.js';
    if (file_exists($js)) {
      drupal_add_js($js);
    }
  //}
 ...
 */
function gas_preprocess_html(&$vars) {
 ...
 */
function gas_process_html(&$vars) {
 ...
 */
function gas_html_head_alter(&$head_elements) {
